...
exports.signup = async (req, res) => {
    try {
        ...
        res.cookie('token', signToken(user._id), { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'strict' });
        res.status(201).json({ id: user._id, name: user.name, email: user.email, credits: user.credits });
...
exports.login = async (req, res) => {
    try {
        ...
        res.cookie('token', signToken(user._id), { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'strict' });
        res.status(200).json({ id: user._id, name: user.name, email: user.email, credits: user.credits });
    } catch (e) { res.status(500).json({ error: e.message }); }
};
